...
const ModalApproved = ({modalApproved, setModalApproved, order, refetchAllTransactionsAdmin}) => {
    ...
    const handleApproveOrder = useMutation(async () => {
        try {
            ...
            const response = await API.patch(`/transaction-admin/${order?.id}`, payload);
        ...
        <>
            <div className="modal-approved-container">
                <div className="content-approved">
                    <Image src={logo} className="img-logo" alt=""/>
                    <p className="status-payment-approved">
                        <h3 className="status-approved">Status Transaction</h3>
                        {order?.status === "pending" && (<Alert variant="warning" style={{width: "200px", height: "40px"}} className="d-inline-block p-1 px-3 fw-bold text-light bg-warning">Waiting Payment</Alert>)}
                        {order?.status === "failed" && (<Alert variant="danger" style={{width: "200px", height: "40px"}} className="d-inline-block p-1 px-3 fw-bold text-light bg-danger">Payment Failed</Alert>)}
                        {order?.status === "reject" && (<Alert variant="danger" style={{width: "200px", height: "40px"}} className="d-inline-block p-1 px-3 fw-bold text-light bg-danger">Transaction Rejected</Alert>)}
                        {order?.status === "success" && (<Alert variant="warning" style={{width: "200px", height: "40px"}} className="d-inline-block p-1 px-3 fw-bold text-light bg-success">Waiting Approved</Alert>)}
                        {order?.status === "approve" && (<Alert variant="success" style={{width: "200px", height: "40px"}} className="d-inline-block p-1 px-3 fw-bold text-light bg-success">Transaction Approved</Alert>)}
                    </p>
                </div>
                <div className="content-img-payment-approved">
                    <img className='img-payment-approved' src={transaction} alt="" />
